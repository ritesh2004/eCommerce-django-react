...
function Address() {
    ...
    const fetchAddress = async () =>{
      let res = await axios.get("http://127.0.0.1:8000/address/");
      for (let index = 0; index < res.data.length; index++) {
        if (res.data[index].username === user['username']) {
          setName(res.data[index].name)
          setEmail(res.data[index].email)
          setUsername(res.data[index].username)
          setContact(res.data[index].contact)
          setAddress1(res.data[index].address1)
          setAddress2(res.data[index].address2)
          setCity(res.data[index].city)
          setState(res.data[index].state)
          setZip(res.data[index].zipcode)
  ...
  return (
    ...
    <h2 className='text-center' style={{marginBottom:'50px'}}>ADDRESS</h2>
      <Row className="mb-3">
      ...
      {register?(<><Button variant="warning" onClick={()=>{navigate(`/updateaddress/${id}/`)}}>
        Update
